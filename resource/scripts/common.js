...
function ajaxshow(url, title, options, events) {
	...
	if(modalobj.length == 0) {
		$(document.body).append('<div id="modal-message" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="position:fixed;top:10%;background:#fff;overflow:visible;height:fit-content;"></div>');
		var modalobj = $('#modal-message');
	}
	html = (typeof title != 'undefined' ? '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="myModalLabel">'+title+'</h3></div>' : '') +
			...
			'<div class="modal-footer">'+(typeof events['confirm'] == 'function' ? '<a href="javascript:;" class="btn btn-primary confirm" data-dismiss="modal" aria-hidden="true">确定</a>' : '') + '<a href="#" class="btn" data-dismiss="modal" aria-hidden="true">关闭</a></div>';
	modalobj.html(html);
    if (typeof option['remote'] != 'undeinfed') {
        modalobj.find('.modal-body').html(getModule(option['remote']));
    }
	if (typeof option['width'] != 'undeinfed' && option['width'] > 0) {
		modalobj.css({'width' : option['width'], 'marginLeft' : ($(window).width() - option['width']) / 2});
	}
    ...
    if (typeof events['shown'] == 'function') {
        events['shown']();
    }
	return modalobj.modal(option);
}
function ajaxpreview(site_url,width,height) {
    if(width==null)width=320;
    if(height==null)height=480;
    var modalobj = $('#modal-preview');
    if(modalobj.length == 0) {
        $(document.body).append('<div id="modal-preview" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="position:fixed;top:10%;background:#fff;overflow:visible"></div>');
        var modalobj = $('#modal-preview');
    }
    html = '<iframe width="100%" scrolling="yes" height="100%" frameborder="0" src="'+site_url+'" id="preview" name="preview" style="width: '+width+'px; overflow: visible; height: '+height+'px;"></iframe><div class="modal-footer" style="background:#fff"><a href="#" class="btn btn-white" data-dismiss="modal" aria-hidden="true">关闭</a></div>';
    modalobj.html(html);
    modalobj.css({'width' : width, 'marginLeft' : ($(window).width()-width)/2});
    modalobj.css({'height' : height});
    modalobj.on('hidden', function(){modalobj.remove();});
    return modalobj.modal({'show' : true});
}
function modelshow(content, title, options, events) {
    $("#modal-message").remove();
    $(".modal-backdrop").remove();
    var modalobj = $('#modal-message');
    var defaultoptions = {backdrop: 'static'};
    var defaultevents = {};
    var option = $.extend({}, defaultoptions, options);
    var events = $.extend({}, defaultevents, events);

    if(modalobj.length == 0) {
        $(document.body).append('<div class="modal fade" id="modal-message"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">Dynamic Content</h4></div><div class="modal-body"></div><div class="modal-footer"><button type="button" class="btn btn-white" data-dismiss="modal">关闭</button><button type="button" class="btn btn-success" data-dismiss="modal">确定</button></div></div></div></div>');
        var modalobj = $('#modal-message');
    }
    modalobj.find(".modal-title").text(title);
    if (typeof option['width'] != 'undeinfed' && option['width'] > 0) {
        modalobj.css({'marginLeft' : -option['width']*1.5});
        modalobj.find('.modal-content').css({'width' : option['width'], 'marginLeft' : option['width'] / 2});
    }
    if (typeof option['height'] != 'undeinfed' && option['height'] > 0) {
        modalobj.find('.modal-content').css({'max-height' : option['height']});
    }
    if (events) {
        for (i in events) {
            if (typeof events[i] == 'function') {
    ...
    modalobj.on('hidden', function(){modalobj.remove();});
    if (typeof events['confirm'] == 'function') {
        modalobj.find('.btn-success', modalobj).on('click', events['confirm']);
    }
    modalobj.find('.modal-body').html(content);
    if (typeof events['shown'] == 'function') {
        events['shown']();
    }
    return modalobj.modal('show', option);
}
/*
	根据html数据创建一个ITEM节点
*/
...
function kindeditor(selector) {
	...
	var option = {
		...
		themeType : 'simple',
		langType : 'zh_CN',
		uploadJson : 'pages/Upload/uploadImage.do',
		resizeType : 1,
		 filterMode : false,
		...
		items : [
			...
			'image', 'multiimage', 'table', 'hr', 'emoticons', 'link', 'unlink','audio','video','weibo','|',
			'preview', 'plainpaste', '|', 'removeformat','source', 'fullscreen'
		]
	}
	if (typeof KindEditor == 'undefined') {
		$.getScript('./resource/script/kindeditor/kindeditor-min.js', function(){
	...
	function initKindeditor(selector, option) {
	 editor = KindEditor.create(selector, option);
...
function colorpicker(obj) {
	...
	obj.spectrum({
		...
		change: function(color) {
			$(this).parent().find("input[type=text]").val(color.toHexString());
		},
		palette: [
