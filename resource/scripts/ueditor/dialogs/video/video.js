...
(function(){
    $.get("/cms/vuploadsign",function (msg) {
        $("#vuploadframe").attr("src","//v-m.zjol.com.cn/a/qiniu/index.shtml?enCodeStr="+msg.enCodeStr+"&sign="+msg.sign);
        $("#dndArea").click(function(){
            window.frames[0].postMessage('upload', window.location.protocol+'//v-m.zjol.com.cn');
        });
        //主页面接收消息
        var myevent=function(e) {
            $(".statusBar").removeClass("element-invisible");
            $(".statusBar .progress").show();
            $(".statusBar .progress .text").text(e.data+"%");
            $(".statusBar .progress .percentage").attr("style","width:"+e.data+"%");
            if(e.data>=100){
                $(".statusBar").addClass("element-invisible");
                $(".statusBar .progress").hide();
                $("#vedioId").val(e.data);
                $("#videopreview").attr("src","//v-static.zjol.com.cn/"+e.data);
                $("#videopreview").attr("poster","//v-img.zjol.com.cn/"+e.data+".jpg_640x360");
                $("#videopreview").attr("vid",e.data);
                $("#videopreview").show();
                $("#dndArea").hide();
            }
        }
        if (window.attachEvent) {
            window.attachEvent("message", myevent);
        } else if (window.addEventListener) {
            window.addEventListener("message", myevent, false);
        }
    });
    var video = {},
        uploadVideoList = [],
    ...
    function insertUpload(){
        ...
        videoObjs.push({
            url: "//v-cdn.zjol.com.cn/index.html?id="+$("#videopreview").attr("vid"),
            width:width,
            height:height,
            align:align
        });
